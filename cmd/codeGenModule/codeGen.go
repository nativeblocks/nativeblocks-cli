	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for key, value := range blocks {
				...
				name := key + "-Block"
				block := generateJSClass(strcase.ToCamel(name), component, "BLOCK")
				if err := fm.SaveByteToFile(strcase.ToCamel(name)+".js", []byte(block)); err != nil {
					return err
				}
			}
			for key, value := range actions {
				...
				name := key + "-Action"
				action := generateJSClass(strcase.ToCamel(name), component, "ACTION")
				if err := fm.SaveByteToFile(strcase.ToCamel(name)+".js", []byte(action)); err != nil {
					return err
				}
			}
